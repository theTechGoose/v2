...
export function sinceBadge(days: number): SinceBadgeData {
  ...
  return {
    tier,
    num: String(weeks),
    unit: weeks === 1 ? "week ago" : "weeks ago",
  };
}
...
export function balanceDisplay(c: CustomerCard): BalanceDisplay {
  if (c.balanceCents > 0) {
    return {
      ...
      text: `${dollars(c.balanceCents)} due`,
    };
  }
  if (c.balanceCents < 0) {
    return {
      cls: "ccard2__bal-val--cred",
      text: `${dollars(-c.balanceCents)} credit`,
    };
  }
...
export function addressFor(c: CustomerCard): string {
  ...
  if (c.segment === "property_mgmt") {
    return `${c.name.split(" ")[0]} property — multiple units`;
  }
...
export function storyLineFor(c: CustomerCard): string {
  ...
  if (c.balanceCents > 0) {
    return `${dollars(c.balanceCents)} outstanding · ${c.balanceSub}.`;
  }
  if (c.status === "active") {
    return `${c.activeJobs} active job${
      c.activeJobs === 1 ? "" : "s"
    } · ${c.jobsSub}.`;
  }
  if (c.status === "lead") return `New lead — last touch ${c.lastWhenRel}.`;
  if (c.status === "cold") {
    return `Quiet for ${c.daysSinceContact} day${
      c.daysSinceContact === 1 ? "" : "s"
    }. Worth a hello.`;
  }
  if (c.status === "regular") {
    return `Regular client. Last activity ${c.lastWhenRel}.`;
  }
