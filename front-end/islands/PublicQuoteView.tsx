/**
 * PublicQuoteView — client-side loader for the public Quote + Agreement page.
 *
 ...
 * skeleton immediately, then fetches `/quotes/:id/public` through the
...
export default function PublicQuoteView(
  { id, lang }: {
     ...
     *  outgoing-comms language (QuoteDoc decides). */
    lang?: Lang;
  },
) {
  const msgLang: Lang = lang ?? "en";
  const LINK_GONE = tFor(msgLang, "publicQuote.linkExpired");
  ...
  useEffect(() => {
    ...
    fetch(`/api/quotes/${id}/public`, {
      ...
      .then(async (r) => {
        if (!alive) return;
        if (r.ok) {
          setS({ phase: "ok", quote: (await r.json()) as QuotePublic });
        } else {
  ...
  if (s.phase === "ok" && s.quote) {
    return <QuoteDoc quote={s.quote} lang={lang} />;
  }
  if (s.phase === "error") {
    return (
      <ErrorCard
        message={s.message ?? tFor(msgLang, "publicQuote.unavailable")}
        lang={msgLang}
      />
    );
  }
