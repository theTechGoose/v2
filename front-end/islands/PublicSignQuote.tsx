 ...
 * Both are submitted to /api/quotes/:id/accept as `signature` (PNG data
 * URL) + `name` — accepting the quote IS the signature ceremony. The typed
 * name is the legal record; when the customer
 ...
 */
export default function PublicSignQuote({ quoteId, lang = "en" }: Props) {
    ...
    >
      {
        /* Pad toolbar — the "YOUR Signature" heading + the "Sign & type name
          ...
          (quote-doc.tsx), so this strip only carries the Undo/Clear
          signature aids. */
      }
      <div
