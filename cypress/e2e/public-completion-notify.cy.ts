 ...
 * Email."
 *
 * After sending, and again after the customer signs (accepting the quote on
 * /q IS the signing), the contractor-facing UI must confirm that BOTH
 * channels went out (the "EMAILED TO … AND TEXTED TO …" receipt strip
 * pattern from the deck).
 */
describe("completion text + email receipts", () => {
  ...
  it("after sending, the quote surface confirms the email AND the text went out", () => {
    ...
    cy.contains(new RegExp(`emailed to ${CUSTOMER_EMAIL}`, "i"), {
      timeout: 10_000,
    })
      .should("be.visible");
  ...
  it("after the customer signs, a completion receipt shows both channels again", () => {
    ...
    cy.visit(`/quotes?open=${quoteId}`);
    cy.contains(/signed|accepted/i, { timeout: 10_000 }).should("be.visible");
