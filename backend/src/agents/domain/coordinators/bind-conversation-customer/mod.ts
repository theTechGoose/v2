 ...
 * bound quote, if any) at a different customer the contractor owns.
 *
 ...
 * lifecycle: phase may already be past `terms`, the terms may be fully
 * drafted, and we don't want to advance any wizard state or append chat
 * messages — just re-point the binding.
 */
@Injectable()
export class BindConversationCustomer {
  ...
  async run(
    input: BindConversationCustomerInput,
  ): Promise<BindConversationCustomerResult> {
    const conv = await this.conversations.get(input.conversationId);
    if (conv.userId !== input.userId) throw new Error("forbidden");

    const customer = await this.customers.getOwned(
      input.customerId,
      input.userId,
    );

    if (conv.quoteId) {
      await this.quotes.update(conv.quoteId, input.userId, {
