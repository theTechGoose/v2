...
import { SendOtp, SendOtpCooldownError } from "./mod.ts";
...
Deno.test("send-otp integration: normalizes phone, generates code, persists OTP record, dispatches SMS", async () => {
  ...
  assert(
    smsCalls[0].body.startsWith("Tu código"),
    `expected Spanish copy, got: ${smsCalls[0].body}`,
  );
  assert(smsCalls[0].body.includes(result.codeForDispatch));
  await resetKv();
});

Deno.test("send-otp integration: defaults to Spanish copy when language omitted (Spanish-first)", async () => {
  Deno.env.set("KV_PATH", ":memory:");
  await resetKv();
  const { smsCalls, flow } = freshFlow();
  await flow.run({ phoneNumber: "+15125551234" });
  assert(smsCalls[0].body.startsWith("Tu código de Paperwork Monster:"));
  await resetKv();
});

Deno.test("send-otp integration: re-send inside the 30s cooldown is rejected (P-03)", async () => {
  Deno.env.set("KV_PATH", ":memory:");
  await resetKv();
  const { flow } = freshFlow();

  await flow.run({ phoneNumber: "+15125551234", language: "en" });
  // An immediate re-send hits the per-phone cooldown — 429 at the HTTP
  // layer, never a silent {sent:true}.
  await assertRejects(
    () => flow.run({ phoneNumber: "+15125551234", language: "en" }),
    SendOtpCooldownError,
  );
  await resetKv();
});

Deno.test("send-otp integration: re-sending after the cooldown rotates the code and PRESERVES attempts (P-03)", async () => {
  Deno.env.set("KV_PATH", ":memory:");
  await resetKv();
  ...
  // Age the pending record past the 30s cooldown so the resend is legal.
  const kv = await getKv();
  const rec = (await kv.get<{ sentAt: string }>(["otp", "+15125551234"]))
    .value!;
  await kv.set(["otp", "+15125551234"], {
    ...rec,
  ...
  const stored = await otps.get("+15125551234");
  // Attempts SURVIVE the resend — a phone locked by MAX_ATTEMPTS can never
  // be reopened by re-sending (mergeOtpRecordOnResend).
  assertEquals(stored?.attempts, 1);
  ...
  assert(
    typeof first.codeForDispatch === "string" &&
      first.codeForDispatch.length === 6,
  );
  await resetKv();
});
...
Deno.test("send-otp integration: invalid phone propagates (and SMS is NOT attempted)", async () => {
  ...
  let threw = false;
  try {
    await flow.run({ phoneNumber: "abc" });
  } catch {
    threw = true;
  }
