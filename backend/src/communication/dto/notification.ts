 ...
 * the surfaced types (quote_sent, quote_accepted, invoice_paid,
 * invoice_overdue, customer_replied, …). Stored per-user
...
export function parseCreateNotification(input: unknown): CreateNotificationDto {
  ...
  if (errors.length) {
    throw new Error(`invalid notification: ${JSON.stringify(errors)}`);
  }
  return dto;
}
...
export function parseUpdateNotification(input: unknown): UpdateNotificationDto {
  ...
  const errors = validateSync(dto);
  if (errors.length) {
    throw new Error(`invalid notification patch: ${JSON.stringify(errors)}`);
  }
  return dto;
}
