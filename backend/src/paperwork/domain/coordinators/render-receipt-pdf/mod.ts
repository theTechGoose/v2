  ...
  async run(input: RenderReceiptPdfInput): Promise<Uint8Array> {
    ...
    pdf.setAuthor(
      contractor?.name ?? businessName ??
        t(lang, "receiptPdf.contractorFallback"),
    );
    pdf.setSubject(
      `Receipt for invoice #${invoice.id.slice(0, 8).toUpperCase()}`,
    );
    ...
    // Brand eyebrow
    const businessLabel =
      (businessName ?? contractor?.name ?? t(lang, "brand.name")).toUpperCase();
    page.drawText(businessLabel, {
      x: M,
      y: H - 50,
      size: 9,
      font: bold,
      color: rgb(0.85, 0.31, 0.31),
    });
    ...
    page.drawText(`#${invoice.id.slice(0, 8).toUpperCase()}`, {
      ...
      size: 24,
      font: bold,
      color: rgb(0.08, 0.28, 0.32),
    });
    page.drawText(
      t(lang, "receiptPdf.confirmed", { date: formatDate(confirmedAt, lang) }),
      {
        x: W - M - 200,
        y: H - 90,
        size: 10,
        font: reg,
        color: rgb(0.42, 0.48, 0.49),
        maxWidth: 200,
      },
    );
    ...
    page.drawRectangle({
      ...
      width: W - 2 * M,
      height: 86,
      color: rgb(0.91, 0.95, 0.89),
      borderColor: rgb(0.65, 0.83, 0.58),
      borderWidth: 1,
    });
    ...
    page.drawText(methodLabel(method, lang), {
      x: W - M - 18 - methodWidth(methodLabel(method, lang), bold, 12),
      y: H - 195,
      size: 12,
      font: bold,
      color: rgb(0.08, 0.28, 0.32),
    ...
    if (reference) {
      ...
      page.drawText(refLine, {
        x: W - M - 18 - methodWidth(refLine, reg, 10),
        y: H - 215,
        size: 10,
        font: reg,
        color: rgb(0.42, 0.48, 0.49),
    ...
    // Invoice / customer block
    let y = H - 280;
    page.drawText(t(lang, "receiptPdf.invoiceHeader"), {
      x: M,
      y,
      size: 9,
      font: bold,
    ...
    page.drawText(t(lang, "receiptPdf.customerHeader"), {
      x: W / 2,
      y,
      size: 9,
      font: bold,
    ...
    if (invoice.installmentIndex && invoice.installmentTotal) {
      page.drawText(
        t(lang, "receiptPdf.installment", {
          index: invoice.installmentIndex,
          total: invoice.installmentTotal,
        }),
        {
          x: M,
          y,
    ...
    const contactLine = [contractor?.phoneNumber, contractor?.email].filter(
      Boolean,
    ).join(" · ");
    if (contactLine) {
    ...
    page.drawText(
      t(lang, "receiptPdf.poweredBy", { brand: t(lang, "brand.name") }),
      {
        x: M,
        y: footerY - 30,
        size: 8,
        font: reg,
        color: rgb(0.66, 0.70, 0.70),
      },
    );
...
function methodLabel(m: PaymentMethod, lang: Lang): string {
  switch (m) {
    case "check":
      return t(lang, "receiptPdf.method.check");
    case "venmo":
      return t(lang, "receiptPdf.method.venmo");
    case "zelle":
      return t(lang, "receiptPdf.method.zelle");
    case "cashapp":
      return t(lang, "receiptPdf.method.cashApp");
    case "paypal":
      return t(lang, "receiptPdf.method.paypal");
    case "cash":
      return t(lang, "receiptPdf.method.cash");
    case "ach":
      return t(lang, "receiptPdf.method.ach");
    case "card":
      return t(lang, "receiptPdf.method.card");
    case "other":
      return t(lang, "receiptPdf.method.other");
  }
}
